...
from note_app.service import create_note_svc,get_note_svc,delete_note_svc, update_note_svc
from note_app.schemas import CreateNote,NoteResponse,UpdateNote
...
def get_note(current_user: User = Depends(get_current_user),db:Session= Depends(get_db)):
    ...
    if not note:
        raise HTTPException(status_code=404,detail="No note yet try add one")
    return note

@note_router.patch("/update/{note_id}",status_code=200,dependencies = [Depends(JWTBearer())])
def update_note(note_data: UpdateNote,note_id:int,current_user: User = Depends(get_current_user),db:Session= Depends(get_db)):
    note = update_note_svc(note_data,current_user,note_id,db)
    if not note:
        raise HTTPException(status_code=404,detail="No note yet try add one")
    return note
